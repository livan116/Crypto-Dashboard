...
import { FaArrowUp, FaArrowDown, FaChartLine, FaWallet, FaExchangeAlt, FaClock, FaChartBar } from 'react-icons/fa';
...
const TopGainers = () => {
    ...
    useEffect(() => {
        if (topMovers?.gainers?.length > 0 && !selectedCoin) {
            handleCoinSelect(topMovers.gainers[0]);
        }
    }, [topMovers]);
    ...
    const totalPages = Math.ceil(topMovers.gainers.length / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    const currentGainers = topMovers.gainers.slice(startIndex, endIndex);

    return (
        <div className="min-h-screen w-full bg-[#0F172A] fixed inset-0 overflow-y-auto text-white">
            <div className="min-h-screen w-full bg-gradient-to-br from-green-500/5 via-gray-900 to-gray-900">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                            <div>
                                ...
                                <p className="text-gray-400 mt-2">
                                    Discover the best performing cryptocurrencies in the market
                                </p>
                            </div>
                            <Link
                                to="/"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="bg-gray-800/30 backdrop-blur-sm border border-gray-700/50 rounded-xl p-4">
                                <div className="flex items-center">
                                    ...
                                    <div>
                                        <p className="text-gray-400 text-sm">Total Gainers</p>
                                        <p className="text-2xl font-bold text-white">{topMovers.gainers.length}</p>
                                    </div>
                                </div>
                            </div>
                            <div className="bg-gray-800/30 backdrop-blur-sm border border-gray-700/50 rounded-xl p-4">
                                <div className="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-2xl font-bold text-white">+{topMovers.gainers[0]?.price_change_percentage_24h.toFixed(2)}%</p>
                                    </div>
                                </div>
                            </div>
                            <div className="bg-gray-800/30 backdrop-blur-sm border border-gray-700/50 rounded-xl p-4">
                                <div className="flex items-center">
                                    ...
                                    <div>
                                        <p className="text-gray-400 text-sm">Average Gain</p>
                                        <p className="text-2xl font-bold text-white">
                                            +{(topMovers.gainers.reduce((acc, curr) => acc + curr.price_change_percentage_24h, 0) / topMovers.gainers.length).toFixed(2)}%
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="space-y-4">
                            <div className="bg-gray-800/30 backdrop-blur-sm border border-gray-700/50 rounded-xl p-4">
                                ...
                                <div className="space-y-3">
                                    {currentGainers.map((crypto, index) => (
                                        <div
                                            key={crypto.id}
                                        ...
                                        >
                                            <div className="absolute -left-2 top-1/2 -translate-y-1/2 w-1 h-8 bg-green-500 rounded-r-full 
                                            ...
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center space-x-3">
                                                    <div className="relative">
                                                        ...
                                                        <div className="absolute -top-2 -right-2 bg-green-500/20 text-green-400 text-xs font-bold px-2 py-1 rounded-full backdrop-blur-sm border border-green-500/50">
                                                            #{startIndex + index + 1}
                                                        </div>
                                                    </div>
                                                    <div>
                                        ...
                                        <div className="flex items-center space-x-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center space-x-3">
                                                    ...
                                                    <span className="inline-flex items-center bg-green-500/20 text-green-400 px-3 py-1 rounded-full text-sm backdrop-blur-sm border border-green-500/50">
                                                        <FaArrowUp className="mr-1" />
